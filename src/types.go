...
type HandlerConsumer interface {
	BookUpdate(message fcxClient.Message)
	TradeTick(msg Match)
	Clear()
}
...
func NewOrder(id string, size string, price string, side string) (*Order, error) {
	...
	if err != nil {
		return &Order{}, errors.Wrap(err, "Alert: Unable to convert side string to side")
	}
	sizeDec, err := decimal.NewFromString(size)
	if err != nil {
		return nil, errors.Wrap(err, "Alert: Unable to convert size to decimal")
	}
	priceDec, err := decimal.NewFromString(price)
	if err != nil {
		return nil, errors.Wrap(err, "Alert: Unable to convert Price to decimal")
	}
...
func ToSide(side string) (common.Side, error) {
	if side == "buy" {
		...
		return common.BidSide, fmt.Errorf("WARNING: Side %s is not supported", side)
	}
}