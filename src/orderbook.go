...
func (b *Book) Remove(order *Order) error {
	...
	if err != nil {
		return fmt.Errorf("Error - Could not remove order %v", order.ToString())
	}
...
func (b *Book) Match(message Match) error {
	...
	if !ok {
		zap.L().Warn("Error - Could not find level at price", zap.String("price", message.Price.String()))
		return nil
	}
	order, err := bl.GetFirstOrder()
	if err != nil {
		return errors.Wrap(err, "Alert - match message is for an order ID that doesn't exist in book")
	}
...
func (b *Book) Change(message Change) error {
	...
	if err != nil {
		zap.L().Error("Error - order change not executed, unable to find order", zap.String("id", message.OrderID))
		return err
	}
	if order.Size != message.OldSize {
		...
		return errors.Wrap(err, "Order unable to execute against current Order Book")
	}
...
func (b *Book) FindOrder(id string, price decimal.Decimal, side common.Side) (*Order, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Unable to find order (%s) at price (%v) due to error (%s)", id, price, err.Error())
	}
	return order, nil
}
