...
var errOrderNotFound = errors.New("ERROR: Unable to find order")
...
func (b *BookSide) Change(message Change) error {
	...
	if err != nil {
		return errors.Wrap(err, "Alert: Unable to process order change: "+message.OrderID)
	}
	if order.Size != message.OldSize {
		zap.L().Error("Unable to reflect upon order book.",
			zap.String("id", message.OrderID),
			zap.String("order", order.ToString()),
			zap.String("oldsize", message.OldSize.String()))
		return errors.New("ERROR: Change Handler is incorrect.")
	}
...
func (bookSide *BookSide) RemoveBookLevel(price decimal.Decimal) error {
	if !bookSide.HasBookLevel(price) {
		return fmt.Errorf("Alert: BookSide %s could not remove book level as book level at price %s does not exist", common.ToString(bookSide.side), price.String())
	}
...
func (bookSide *BookSide) GetTopLevel() (*BookLevel, error) {
	if bookSide.PriceToLevels.Empty() {
		return nil, fmt.Errorf("Alert: BookSide %s is empty. Cound not level", common.ToString(bookSide.side))
	}
	if bookSide.side == common.BidSide {
		_, level := bookSide.PriceToLevels.Max()
		retVal, ok := level.(*BookLevel)
		if !ok {
			return nil, fmt.Errorf("WARNING: PriceToLevels corrupt level for bookSide %s. Unable to level", common.ToString(bookSide.side))
		}
		return retVal, nil
	} else {
		...
		retVal, ok := level.(*BookLevel)
		if !ok {
			return nil, fmt.Errorf("WARNING: PriceToLevels has a corrupt level for bookSide %s. Tried to get top level", common.ToString(bookSide.side))
		}
		return retVal, nil
	}
}
...
func (bookSide *BookSide) FindOrder(id string, price decimal.Decimal) (*Order, error) {
	...
	if !ok {
		...
		return nil, errors.New("Alert: Could not find level")
	}
	order, err := level.Get(id)
	if err != nil {
		return nil, fmt.Errorf("Alert: Could not find order (%s) at price (%v) due to error (%s)", id, price, err.Error())
	}
	return order, nil
